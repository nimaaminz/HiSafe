...
namespace AForge.Video.DirectShow
{
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        private void OnNewFrame_nima(double sampleTime, IntPtr buffer, int bufferLen)
        {
            //if ((!stopEvent.WaitOne(0, false)) && (NewFrame != null))
                //NewFrame(this, new NewFrameEventArgs(sampleTime, buffer, bufferLen));
        }
        ...
        private class Grabber : ISampleGrabberCB
        {
            ...
            // 18-11-2021 Edit by nima 
            // im trying to send these data directly to my program and
            // use them cause i need a super fast process 
            // i just wanted to get directly the buffers but its not good not working well 
            public int BufferCB_Nima( double sampleTime, IntPtr buffer, int bufferLen)
            {
                if (parent.NewFrame != null)
                {
                    //parent.OnNewFrame(sampleTime, buffer, bufferLen);
                }
                return 0;
            }
        ...
        /// <summary>
        /// set camera properties like brightness and etc 
        /// this added by me NIMA at november 2021 for hadrontavan programmer project
        /// </summary>
